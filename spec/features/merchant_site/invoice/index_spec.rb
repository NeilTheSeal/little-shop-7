...
RSpec.describe 'merchant_invoices#index', type: :feature do
  before(:each) do
    ...
    @item_list.each_with_index do |item, index|
      if index < 10
        @invoice_item_list << create(:invoice_item, item:, invoice: @invoice_list[index], unit_price: item.unit_price, status: "packaged")
      elsif index < 15
        @invoice_item_list << create(:invoice_item, item:, invoice: @invoice_list[index], unit_price: item.unit_price, status: "pending")
      else 
        @invoice_item_list << create(:invoice_item, item:, invoice: @invoice_list[index], unit_price: item.unit_price, status: "shipped")
      end
    end
  ...
  it "merchant invoices link to its show page" do
    ...
    within '.merchant_invoices' do
      ...
      expect(current_path).to eq(merchant_invoice_path(@merchant, @invoice_list[1]))
    end
  end
end